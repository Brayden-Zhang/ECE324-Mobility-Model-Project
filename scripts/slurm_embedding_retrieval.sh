#!/usr/bin/env bash
# Embedding retrieval evaluation (kNN).

#SBATCH --job-name=trajfm-retrieval
...
#SBATCH --output=slurm-retrieval-%j.out
...
if [[ ! -f "${ROOT_DIR}/src/route_rangers/cli/run_embedding_retrieval.py" ]]; then
...
OUTPUT="${OUTPUT:-${ROOT_DIR}/cache/embedding_retrieval_${SLURM_JOB_ID}.json}"
OUTPUT="${OUTPUT//%j/${SLURM_JOB_ID}}"

PYTHONPATH=src python -m route_rangers.cli.run_embedding_retrieval \
