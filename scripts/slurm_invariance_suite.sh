#!/usr/bin/env bash
# Invariance/robustness suite on a checkpoint.

#SBATCH --job-name=trajfm-invar
...
#SBATCH --output=slurm-invar-%j.out
...
if [[ ! -f "${ROOT_DIR}/scripts/run_invariance_suite.py" ]]; then
...
OUTPUT="${OUTPUT:-${ROOT_DIR}/cache/invariance_suite_${SLURM_JOB_ID}.json}"
OUTPUT="${OUTPUT//%j/${SLURM_JOB_ID}}"

python scripts/run_invariance_suite.py \
  --checkpoint "${CKPT_PATH}" \
  --local_data "${LOCAL_DATA}" \
  --output "${OUTPUT}" \
  --batch_size "${BATCH_SIZE:-32}" \
  --max_len "${MAX_LEN:-200}" \
  --prefix_ratios "${PREFIX_RATIOS:-0.25,0.5,0.75,1.0}" \
  --time_shifts_sec "${TIME_SHIFTS_SEC:-0,43200,86400}" \
  --downsample_rates "${DOWNSAMPLE_RATES:-0.5,0.25}"
