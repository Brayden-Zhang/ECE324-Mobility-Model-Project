#!/usr/bin/env bash
# Run benchmark suite on the latest checkpoint.

#SBATCH --job-name=trajfm-bench-latest
...
#SBATCH --output=slurm-bench-latest-%j.out
...
if [[ -z "${CKPT_PATH}" ]]; then
  echo "ERROR: no checkpoint found for glob: ${CKPT_GLOB}" >&2
  exit 1
fi
