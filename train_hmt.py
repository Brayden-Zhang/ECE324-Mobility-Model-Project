...
from route_rangers.models.hmt import HMTConfig, HMTTokenizer, TimeFeatures  # noqa: E402
from route_rangers.data.hmt_dataset import (  # noqa: E402
    WorldTraceIterableDataset,
    WorldTraceMapDataset,
    WorldTraceZipIterableDataset,
    collate_batch,
)
from route_rangers.models.hmt_model import TrajectoryFMHMT  # noqa: E402
from route_rangers.models.flow import (  # noqa: E402
    sample_rectified_flow_targets,
    flow_matching_loss,
)
from route_rangers.data.context import OSMContextIndex, context_tensor_from_index  # noqa: E402
from route_rangers.data.macro_dataset import MacroDistributionDataset  # noqa: E402
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="hf_zip",
        choices=["hf_zip", "hf_stream", "local"],
    )
    ...
    parser.add_argument("--tokenizer", type=str, default="h3", choices=["h3"])
    ...
    parser.add_argument("--graph_long_stride", type=int, default=0)
    parser.add_argument("--graph_long_hops", type=int, default=0)
    parser.add_argument("--graph_global_hubs", type=int, default=0)
    parser.add_argument("--step_attention_window", type=int, default=0)
    parser.add_argument(
        "--graph_same_region", dest="graph_same_region", action="store_true"
    )
    parser.add_argument(
        "--no_graph_same_region", dest="graph_same_region", action="store_false"
    )
    parser.set_defaults(graph_same_region=True)

    parser.add_argument(
        "--space_time_encoder", dest="space_time_encoder", action="store_true"
    )
    parser.add_argument(
        "--no_space_time_encoder", dest="space_time_encoder", action="store_false"
    )
    ...
    parser.add_argument(
        "--use_trip_features", dest="use_trip_features", action="store_true"
    )
    parser.add_argument(
        "--no_trip_features", dest="use_trip_features", action="store_false"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "normalize token loss per-trajectory length (Length-Balanced Token Loss)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--distance_weighted_token_loss",
        action="store_true",
        help=(
            "weight masked token supervision by distance to nearest visible step "
            "to strengthen long-range context learning"
        ),
    )
    parser.add_argument("--distance_loss_power", type=float, default=1.0)
    parser.add_argument("--distance_loss_min_weight", type=float, default=1.0)
    parser.add_argument("--distance_loss_max_weight", type=float, default=4.0)
...
def build_macro_loaders(args):
    ...
    val_loader = (
        DataLoader(
            val_ds, batch_size=args.macro_batch_size, shuffle=False, num_workers=0
        )
        if val_n > 0
        else None
    )
    test_loader = (
        DataLoader(
            test_ds, batch_size=args.macro_batch_size, shuffle=False, num_workers=0
        )
        if test_n > 0
        else None
    )
...
def build_dataloaders(args, take_override=None):
    ...
    if data_mode == "local":
        ...
        train_ds = WorldTraceMapDataset(
            train_samples, max_len=args.max_len, mask_ratio=args.mask_ratio
        )
        val_ds = WorldTraceMapDataset(
            val_samples, max_len=args.max_len, mask_ratio=args.mask_ratio
        )
        test_ds = WorldTraceMapDataset(
            test_samples, max_len=args.max_len, mask_ratio=args.mask_ratio
        )
        return (
            make_dataloader(args, train_ds),
            make_dataloader(args, val_ds),
            make_dataloader(args, test_ds),
        )
...
def scheduled_ratio(
    step: int, min_ratio: float, max_ratio: float, curriculum_steps: int
) -> float:
...
def sample_mask_indices(
    ...
    mask_idx = torch.full(
        (bsz, max_m), -1, dtype=torch.long, device=attention_mask.device
    )
...
def apply_coordinate_noise(
    coords: torch.Tensor, attention_mask: torch.Tensor, noise_std: float
) -> torch.Tensor:
    if noise_std <= 0:
        return coords
...
def masked_cross_entropy_with_token_weights(
    logits: torch.Tensor,
    ...
    weights = token_weights.to(loss.dtype) * valid.float()
    denom = weights.sum().clamp(min=1e-6)
    return (loss * weights).sum() / denom


def spatial_mask_distance_weights(
    coords: torch.Tensor,
    attention_mask: torch.Tensor,
    ...
    weights = torch.zeros((bsz, seq_len), device=coords.device, dtype=coords.dtype)
    valid = attention_mask.bool()
    masked = masked_positions.bool() & valid
    lo = float(min(min_weight, max_weight))
    hi = float(max(min_weight, max_weight))
    exponent = max(0.0, float(power))

    for b in range(bsz):
        m_idx = masked[b].nonzero(as_tuple=False).squeeze(-1)
        if m_idx.numel() == 0:
            continue
        obs_idx = (valid[b] & ~masked_positions[b].bool()).nonzero(as_tuple=False).squeeze(
            -1
        )
        if obs_idx.numel() == 0:
            weights[b, m_idx] = hi
        ...
        d = torch.cdist(coords[b, m_idx], coords[b, obs_idx]).min(dim=1).values
        d_max = d.max().clamp(min=1e-6)
        d_norm = (d / d_max).clamp(0.0, 1.0)
        w = lo + (hi - lo) * d_norm.pow(exponent)
        weights[b, m_idx] = w
    return weights
...
def flow_consistency_loss(
    meso_logits: torch.Tensor, micro_flows: torch.Tensor, region_mask: torch.Tensor
):
...
def compute_trip_features(
    coords: torch.Tensor, attention_mask: torch.Tensor
) -> torch.Tensor:
    ...
    feats = torch.stack(
        [progress_step, progress_dist, trip_len_log, total_dist_log], dim=-1
    )
    return feats
...
def evaluate(model, tokenizer, time_encoder, dataloader, args, context_index=None):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            tokens_l0, tokens_l1, tokens_l2, _vq_loss = tokenizer(
                coords,
                timestamps,
                context,
            ...
            trip_features = (
                compute_trip_features(coords, attention)
                if args.use_trip_features
                else None
            )
            ...
            with torch.autocast(
                ...
                token_weights = None
                if args.distance_weighted_token_loss:
                    token_weights = spatial_mask_distance_weights(
                        coords=coords,
                        attention_mask=attention,
                        masked_positions=mask,
                        power=args.distance_loss_power,
                        min_weight=args.distance_loss_min_weight,
                        max_weight=args.distance_loss_max_weight,
                    )

                if token_weights is not None:
                    token_loss = (
                        masked_cross_entropy_with_token_weights(
                            outputs["step_logits"]["l0"], tokens_l0, mask, token_weights
                        )
                        + masked_cross_entropy_with_token_weights(
                            outputs["step_logits"]["l1"], tokens_l1, mask, token_weights
                        )
                        + masked_cross_entropy_with_token_weights(
                            outputs["step_logits"]["l2"], tokens_l2, mask, token_weights
                        )
                    )
                elif args.length_weighted_loss:
                    token_loss = (
                        masked_cross_entropy_weighted(
                            outputs["step_logits"]["l0"], tokens_l0, mask
                        )
                        + masked_cross_entropy_weighted(
                            outputs["step_logits"]["l1"], tokens_l1, mask
                        )
                        + masked_cross_entropy_weighted(
                            outputs["step_logits"]["l2"], tokens_l2, mask
                        )
                    )
                else:
                    token_loss = (
                ...
                region_loss = masked_cross_entropy(
            ...
            dest_top5 = (
                (dest_topk == dest_targets.unsqueeze(-1))
                .any(dim=-1)
                .float()
                .mean()
                .item()
...
def main():
    ...
    tokenizer = HMTTokenizer(
        tokenizer_cfg, feature_dim=feature_dim, embed_dim=args.embed_dim
    ).to(args.device)
    ...
    model = TrajectoryFMHMT(
        ...
        graph_long_stride=args.graph_long_stride,
        graph_long_hops=args.graph_long_hops,
        graph_global_hubs=args.graph_global_hubs,
    ...
    for epoch in range(args.epochs):
        for micro_step, batch in enumerate(train_loader):
            ...
            trip_features = (
                compute_trip_features(coords_in, attention)
                if args.use_trip_features
                else None
            )
            ...
            if should_step:
                ...
                if step > 0 and step % args.eval_interval == 0:
                    metrics = evaluate(
                        model, tokenizer, time_encoder, val_loader, args, context_index
                    )
                ...
                if step > 0 and step % args.save_interval == 0:
                    ...
                    torch.save(
                        ckpt, Path("checkpoints") / f"{args.ckpt_prefix}_step_{step}.pt"
                    )
    ...
    test_metrics = evaluate(
        model, tokenizer, time_encoder, test_loader, args, context_index
    )
