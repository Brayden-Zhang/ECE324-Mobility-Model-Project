...
class RectifiedFlowHead(nn.Module):
    ...
    def forward(self, hidden: torch.Tensor, x_t: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
...
def sample_rectified_flow_targets(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    # x: [B, L, 2]
    b, l, _ = x.shape
...
def flow_matching_loss(pred_v: torch.Tensor, target_v: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    # mask: [B, L]
