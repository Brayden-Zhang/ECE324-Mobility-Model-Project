...
class TrajectoryFMHMT(nn.Module):
    def __init__(
        ...
        graph_long_stride: int = 0,
        graph_long_hops: int = 0,
        graph_global_hubs: int = 0,
        ...
        self.graph_long_stride = graph_long_stride
        self.graph_long_hops = graph_long_hops
        self.graph_global_hubs = graph_global_hubs
    ...
    def build_step_graph(
        ...
        for b in range(bsz):
            ...
            # Dilated temporal shortcuts add long-range paths without full dense attention.
            if self.graph_long_stride > 0 and self.graph_long_hops > 0:
                for t in range(vlen):
                    i = valid_idx[t].item()
                    for hop in range(1, self.graph_long_hops + 1):
                        span = hop * self.graph_long_stride
                        left = t - span
                        right = t + span
                        if left >= 0:
                            j = valid_idx[left].item()
                            graph[b, i, j] = True
                            graph[b, j, i] = True
                        if right < vlen:
                            j = valid_idx[right].item()
                            graph[b, i, j] = True
                            graph[b, j, i] = True

            # Hub nodes connect distant portions of the trajectory with short path length.
            if self.graph_global_hubs > 0 and vlen > 1:
                hub_count = min(self.graph_global_hubs, vlen)
                if hub_count == 1:
                    hub_pos = [0]
                else:
                    hub_pos = torch.linspace(
                        0,
                        vlen - 1,
                        steps=hub_count,
                        device=valid_idx.device,
                    ).round().long().tolist()
                hubs = valid_idx[hub_pos]
                graph[b, valid_idx.unsqueeze(1), hubs.unsqueeze(0)] = True
                graph[b, hubs.unsqueeze(1), valid_idx.unsqueeze(0)] = True
