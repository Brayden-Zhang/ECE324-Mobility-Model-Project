...
def parse_args():
    ...
    parser.add_argument(
        "--regression_train_max_points",
        type=int,
        default=120000,
        help=(
            "Max number of train points used by regression probe. "
            "0 disables subsampling."
        ),
    )
    parser.add_argument(
        "--regression_val_max_points",
        type=int,
        default=40000,
        help=(
            "Max number of val points used by regression probe and calibration. "
            "0 disables subsampling."
        ),
    )
    parser.add_argument(
...
def _train_regression_head(
    ...
    x_std = train_x.std(dim=0, keepdim=True).clamp(min=1e-6)
    ...
    for _ in range(max(1, epochs)):
        perm = torch.randperm(train_xn.shape[0], device=device)
        train_xn = train_xn[perm]
        train_yn = train_yn[perm]
        for start in range(0, train_xn.shape[0], batch_size):
            xb = train_xn[start : start + batch_size]
            yb = train_yn[start : start + batch_size]
            pred = head(xb)
            loss = torch.nn.functional.smooth_l1_loss(pred, yb, beta=1.0)
            opt.zero_grad(set_to_none=True)
            loss.backward()
            opt.step()
        with torch.no_grad():
            val_loss = torch.nn.functional.smooth_l1_loss(
                head(val_xn), val_yn, beta=1.0
            ).item()
    ...
    head._x_mean = x_mean.cpu()
    head._x_std = x_std.cpu()
...
def _predict_with_head(
    ...
    x = x.to(device).float()
    x_mean = head._x_mean.to(device)
    x_std = head._x_std.to(device)
    x = (x - x_mean) / x_std
    with torch.no_grad():
        pred_n = head(x)
...
def evaluate_task_regression(
    ...
    tr_max = int(getattr(args, "regression_train_max_points", 0) or 0)
    va_max = int(getattr(args, "regression_val_max_points", 0) or 0)
    train_x_fit, train_anchor_fit, train_y_fit = _subsample_triplet(
        train_x,
        train_anchor,
        train_y,
        tr_max,
        seed=getattr(args, "seed", 42) + 101,
    )
    val_x_fit, val_anchor_fit, val_y_fit = _subsample_triplet(
        val_x,
        val_anchor,
        val_y,
        va_max,
        seed=getattr(args, "seed", 42) + 202,
    )

    # Learn residuals over a trajectory-aware interpolation anchor.
    train_res = train_y_fit - train_anchor_fit
    val_res = val_y_fit - val_anchor_fit
    ...
    # Robustness guardrail: clip residual corrections to a high train quantile.
    # This prevents a few unstable predictions from dominating RMSE under noise/drop.
    clip_abs = torch.quantile(train_res.abs(), 0.995, dim=0)
    clip_abs = torch.clamp(clip_abs, min=1e-4)

    with torch.no_grad():
        val_pred_res = _predict_with_head(head, val_x_fit, args.device)
    val_pred_res = torch.clamp(val_pred_res, min=-clip_abs, max=clip_abs)
    d_val_base = _distance_m(val_anchor_fit, val_y_fit, coord_mode)
    mae_val_base = float(d_val_base.mean().item()) if d_val_base.numel() > 0 else 0.0

    # Calibrate residual strength on validation instead of binary on/off.
    # This is more stable under noisy inputs and distribution shifts.
    candidate_alphas = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25]
    best_alpha = 0.0
    best_alpha_mae = mae_val_base
    for alpha in candidate_alphas:
        val_pred = val_anchor_fit + (val_pred_res * alpha)
        d_val = _distance_m(val_pred, val_y_fit, coord_mode)
        mae_val = float(d_val.mean().item()) if d_val.numel() > 0 else float("inf")
    ...
    for split_name, sx, sb, sy in [
        ...
        pred_res = _predict_with_head(head, sx, args.device)
        pred_res = torch.clamp(pred_res, min=-clip_abs, max=clip_abs)
        pred = sb + (pred_res * best_alpha)
    ...
    results["_meta"] = {
        "strategy": "anchor_interpolation_plus_calibrated_residual",
        "residual_alpha": best_alpha,
        "residual_clip_abs": [float(clip_abs[0].item()), float(clip_abs[1].item())],
        "train_points_fit": int(train_x_fit.shape[0]),
        "val_points_fit": int(val_x_fit.shape[0]),
        "val_anchor_mae_m": mae_val_base,
        "val_residual_mae_m": best_alpha_mae,
    }
    return results
