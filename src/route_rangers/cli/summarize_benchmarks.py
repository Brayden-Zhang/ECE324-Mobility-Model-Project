...
def main():
    ...
    for f in args.files:
        ...
        for split_name, split_payload in payload.get("splits", {}).items():
            next_top1 = get_metric(
                split_payload, ["next_location_probe", "test", "top1"]
            )
            ...
            next_mae = get_metric(
                split_payload, ["next_location_regression_probe", "test", "mae_m"]
            )
            next_rmse = get_metric(
                split_payload, ["next_location_regression_probe", "test", "rmse_m"]
            )
            dest_mae = get_metric(
                split_payload, ["destination_regression_probe", "test", "mae_m"]
            )
            dest_rmse = get_metric(
                split_payload, ["destination_regression_probe", "test", "rmse_m"]
            )
            print(
