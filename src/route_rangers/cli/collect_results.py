...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional secondary markdown file to update with the results block.",
    )
...
def summarize_travel_time(cache_dir: str) -> str:
    ...
    if not obj:
        return "No travel-time estimation results yet."
    lines = [f"latest: `{Path(path).name}`"]
    results = obj.get("prefix_ratios") or obj.get("results") or {}
    if results:
        header = (
        ...
        for ratio, metrics in sorted(results.items(), key=lambda x: float(x[0])):
            ...
            lines.append(
                f"| {ratio} | {test_metrics.get('mae_s', test_metrics.get('mae_seconds', 0)):.1f} | "
                f"{test_metrics.get('rmse_s', test_metrics.get('rmse_seconds', 0)):.1f} | "
                f"{test_metrics.get('mape', 0):.3f} | "
                f"{test_metrics.get('r2', 0):.3f} |"
            )
    return "\n".join(lines)
...
def summarize_anomaly_detection(cache_dir: str) -> str:
    ...
    if not obj:
        return "No anomaly detection results yet."
    lines = [f"latest: `{Path(path).name}`"]
    results = obj.get("anomaly_types") or obj.get("results") or {}
    if results:
        header = "| anomaly_type | AUROC | PR-AUC |\n|---|---|---|"
        lines.append(header)
        for atype in ("noise", "reverse", "swap", "detour", "combined"):
            m = results.get(atype, {}) if atype != "combined" else obj.get("combined", {})
            if m:
                lines.append(
...
def summarize_trip_classification(cache_dir: str) -> str:
    ...
    if not obj:
        return "No trip classification results yet."
    lines = [f"latest: `{Path(path).name}`"]
    results = obj.get("tasks") or obj.get("results") or {}
    if results:
        header = "| task | top1 | top5 | n_classes |\n|---|---|---|---|"
        lines.append(header)
        for task_name in ("speed_mode", "duration_bucket", "distance_bucket"):
            ...
            if m:
                test = m.get("test", m)
                lines.append(
                    f"| {task_name} | {test.get('top1', test.get('accuracy', 0)):.3f} | "
                    f"{test.get('top5', 0):.3f} | {m.get('num_classes', m.get('n_classes', 0))} |"
                )
    return "\n".join(lines)
...
def summarize_similarity_retrieval(cache_dir: str) -> str:
    ...
    lines = [f"latest: `{Path(path).name}`"]
    sections = {
        "geographic_consistency": obj.get("geographic_consistency", {}),
        "self_retrieval": obj.get("self_retrieval", {}),
    }
...
def summarize_next_poi_eval(cache_dir: str) -> str:
    files = latest_files(os.path.join(cache_dir, "next_poi_eval_*.json"))
    if not files:
        return "No next-POI results yet."
    path = files[-1]
    obj = _safe_read_json(path)
    if not obj:
        return "No next-POI results yet."
    ...
    next_poi = results.get("next_poi", {})
    test = next_poi.get("test", {})
    if test:
        lines.append(
            "- next-poi: "
            f"top1={test.get('top1', 0):.3f}, "
            f"top5={test.get('top5', 0):.3f}, "
            f"top10={test.get('top10', 0):.3f}, "
            f"ndcg10={test.get('ndcg10', 0):.3f}, "
            f"mrr={test.get('mrr', 0):.3f}"
        )

    uid = results.get("user_identification", {}).get("test", {})
    if uid:
        lines.append(f"- user-identification top1={uid.get('top1', 0):.3f}")
    return "\n".join(lines)


def summarize_cross_city_transfer(cache_dir: str) -> str:
    files = latest_files(os.path.join(cache_dir, "cross_city_transfer_*.json"))
    if not files:
        return "No cross-city transfer results yet."
    path = files[-1]
    obj = _safe_read_json(path)
    if not obj:
        return "No cross-city transfer results yet."
    ...
    top1 = np_agg.get("top1", {})
    top5 = np_agg.get("top5", {})
    top10 = np_agg.get("top10", {})
    ndcg10 = np_agg.get("ndcg10", {})
    mrr = np_agg.get("mrr", {})
    if top1:
        lines.append(
            "- next-poi mean±std: "
            f"top1={top1.get('mean', 0):.3f}±{top1.get('std', 0):.3f}, "
            f"top5={top5.get('mean', 0):.3f}±{top5.get('std', 0):.3f}, "
            f"top10={top10.get('mean', 0):.3f}±{top10.get('std', 0):.3f}, "
            f"ndcg10={ndcg10.get('mean', 0):.3f}±{ndcg10.get('std', 0):.3f}, "
            f"mrr={mrr.get('mean', 0):.3f}±{mrr.get('std', 0):.3f}"
        )
    uid = agg.get("user_identification", {}).get("top1", {})
    if uid:
        lines.append(
            f"- user-identification top1 mean±std={uid.get('mean', 0):.3f}±{uid.get('std', 0):.3f}"
        )
    return "\n".join(lines)
...
def main():
    ...
    next_poi_md = summarize_next_poi_eval(cache_dir)
    cross_city_md = summarize_cross_city_transfer(cache_dir)
    ...
    # Update paper draft
    paper_path = Path(args.paper_doc) if args.paper_doc else None
