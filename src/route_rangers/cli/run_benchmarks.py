...
def _is_valid_latlon(lat: np.ndarray, lon: np.ndarray) -> bool:
    ...
    lon_f = lon[finite]
    return (
        float(np.max(np.abs(lat_f))) <= 90.0 and float(np.max(np.abs(lon_f))) <= 180.0
    )
...
def _orientation_score(lat: np.ndarray, lon: np.ndarray) -> float:
    ...
    if lat.shape[0] > 256:
        idx = (
            np.linspace(0, lat.shape[0] - 1, num=256, endpoint=True)
            .round()
            .astype(np.int64)
        )
        lat = lat[idx]
...
def split_indices(
    dataset: FixedTrajectoryDataset, mode: str, seed: int
...
def load_backbone(
    ...
    base_feature_dim = 17
    context_dim = (
        ckpt_args.get("osm_context_dim", 0) if ckpt_args.get("osm_context") else 0
    )
    feature_dim = (
        base_feature_dim + context_dim if context_dim > 0 else base_feature_dim
    )

    tokenizer = HMTTokenizer(
        tokenizer_cfg, feature_dim=feature_dim, embed_dim=ckpt_args["embed_dim"]
    ).to(device)
    ...
    if "time_encoder" in ckpt:
        safe_load_state_dict(
            time_encoder, ckpt["time_encoder"], module_name="time_encoder"
        )
    else:
        print(
            "warning: checkpoint missing time_encoder state; using fresh initialization"
        )
...
def sample_mask(
    attention_mask: torch.Tensor, ratio: float, generator: torch.Generator
) -> torch.Tensor:
...
def collect_next_location_features(
    ...
    for batch in loader:
        ...
        if max_points > 0 and seen >= max_points:
            break
    if not feats:
        return torch.zeros((0, pack.ckpt_args["embed_dim"])), torch.zeros(
            (0,), dtype=torch.long
        )
...
def collect_destination_features(
    ...
    for batch in loader:
        ...
        if max_points > 0 and seen >= max_points:
            break
    if not feats:
        return torch.zeros((0, pack.ckpt_args["embed_dim"] * 2)), torch.zeros(
            (0,), dtype=torch.long
        )
    x = torch.cat(feats, dim=0)
...
def haversine_m(pred_latlon: torch.Tensor, true_latlon: torch.Tensor) -> torch.Tensor:
    ...
    c = 2.0 * torch.atan2(
        torch.sqrt(a.clamp(min=0.0, max=1.0)), torch.sqrt((1.0 - a).clamp(min=0.0))
    )
    return 6371000.0 * c
...
def train_regression_probe(
    ...
    for _ in range(max(1, epochs)):
        if train_xn.shape[0] > 0:
            ...
            for start in range(0, train_xn.shape[0], batch_size):
                ...
                xb = train_xn[start:end]
                yb = train_y_n[start:end]
                pred_n = head(xb)
                loss = torch.nn.functional.smooth_l1_loss(pred_n, yb, beta=1.0)
                opt.zero_grad(set_to_none=True)
                loss.backward()
                opt.step()

        with torch.no_grad():
            val_pred_n = head(val_xn)
            val_loss = torch.nn.functional.smooth_l1_loss(
                val_pred_n, val_y_n, beta=1.0
            ).item()
    ...
    with torch.no_grad():
        train_pred = head(train_xn) * y_std + y_mean
        val_pred = head(val_xn) * y_std + y_mean
        test_pred = head(test_xn) * y_std + y_mean
...
def train_probe(
    ...
    with torch.no_grad():
        train_metrics = evaluate_logits(head(train_xn), train_y)
        val_metrics = evaluate_logits(head(val_xn), val_y)
        test_metrics = evaluate_logits(head(test_xn), test_y)
    return {"train": train_metrics, "val": val_metrics, "test": test_metrics}
...
def run_split_suite(
    ...
    print(
        f"[{name}] sizes train={len(train_idx)} val={len(val_idx)} test={len(test_idx)}"
    )
